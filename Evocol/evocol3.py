...
def algorytm_genetyczny(graf,n):
    ...
    t=1000
    while(t):
        print("Iteracja: ", t, "Najlepszy kolor: ", len(set(best_osobnik[0])), "Liczba błędów: ", sum(best_osobnik[2]), "Srednia ocena: ", sum([f for _,f,_ in Populacja])/len(Populacja), "         Srednia liczba kolorow",sum([len(set(f)) for f,_,_ in Populacja])/len(Populacja))
        t-=1
